...
* 射频开关：T-Impulse-Plus 使用 AcSiP 控制模式 A，由 nRF52840 直接控制 `RF_VC1`（`P1.13`）和 `RF_VC2`（`P1.07`）。`DIO2`（`P1.15`）是单独引出的信号，不能替代这两个控制引脚。发射时设置 `RF_VC1/RF_VC2` 为 `HIGH/LOW`，接收时设置为 `LOW/HIGH`。
